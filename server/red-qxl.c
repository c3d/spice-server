...
int red_qxl_check_qxl_version(QXLInstance *qxl, int major, int minor)
{
    ...
    return ((qxl_major > major) || ((qxl_major == major) && (qxl_minor >= minor)));
}

static void red_qxl_set_display_peer(RedChannel *            channel,
                                     RedClient *             client,
                                     RedStream *             stream,
                                     int                     migration,
                                     RedChannelCapabilities *caps)
{
    ...
    dispatcher_send_message(dispatcher, RED_WORKER_MESSAGE_DISPLAY_CONNECT, &payload);
}
...
static void red_qxl_disconnect_display_peer(RedChannelClient *rcc)
{
    ...
    // TODO: we turned it to be sync, due to client_destroy . Should we support
    // async? - for this we will need ref count
    // for channels
    dispatcher_send_message(dispatcher, RED_WORKER_MESSAGE_DISPLAY_DISCONNECT, &payload);
}
...
static void red_qxl_display_migrate(RedChannelClient *rcc)
{
    ...
    dispatcher_send_message(dispatcher, RED_WORKER_MESSAGE_DISPLAY_MIGRATE, &payload);
}

static void red_qxl_set_cursor_peer(RedChannel *            channel,
                                    RedClient *             client,
                                    RedStream *             stream,
                                    int                     migration,
                                    RedChannelCapabilities *caps)
{
    ...
    dispatcher_send_message(dispatcher, RED_WORKER_MESSAGE_CURSOR_CONNECT, &payload);
}
...
static void red_qxl_disconnect_cursor_peer(RedChannelClient *rcc)
{
    ...
    dispatcher_send_message(dispatcher, RED_WORKER_MESSAGE_CURSOR_DISCONNECT, &payload);
}
...
static void red_qxl_cursor_migrate(RedChannelClient *rcc)
{
    ...
    dispatcher  = (Dispatcher *)g_object_get_data(G_OBJECT(channel), "dispatcher");
    payload.rcc = g_object_ref(rcc);
    dispatcher_send_message(dispatcher, RED_WORKER_MESSAGE_CURSOR_MIGRATE, &payload);
}

static void red_qxl_update_area(QXLState *qxl_state,
                                uint32_t  surface_id,
                                QXLRect * qxl_area,
                                ...
                                uint32_t  clear_dirty_region)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_UPDATE, &payload);
}
                                   ...
                                   uint32_t   clear_dirty_region)
{
    ...
    red_qxl_update_area(qxl_state, surface_id, qxl_area, qxl_dirty_rects, num_dirty_rects,
                        clear_dirty_region);
}
...
static void red_qxl_add_memslot(QXLState *qxl_state, QXLDevMemSlot *mem_slot)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_ADD_MEMSLOT, &payload);
}
...
static void red_qxl_destroy_surfaces(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_DESTROY_SURFACES, &payload);
}
...
static void red_qxl_destroy_primary_surface_sync(QXLState *qxl_state, uint32_t surface_id)
{
    RedWorkerMessageDestroyPrimarySurface payload;
    payload.surface_id = surface_id;
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_DESTROY_PRIMARY_SURFACE,
                            &payload);
    red_qxl_destroy_primary_surface_complete(qxl_state);
}

static void
red_qxl_destroy_primary_surface_async(QXLState *qxl_state, uint32_t surface_id, uint64_t cookie)
{
...
static void red_qxl_destroy_primary_surface(QXLState *qxl_state,
                                            uint32_t  surface_id,
                                            int       async,
                                            uint64_t  cookie)
{
...
static void red_qxl_create_primary_surface_async(QXLState *           qxl_state,
                                                 uint32_t             surface_id,
...
static void red_qxl_create_primary_surface_sync(QXLState *           qxl_state,
                                                uint32_t             surface_id,
                                                QXLDevSurfaceCreate *surface)
{
    ...
    payload.surface_id = surface_id;
    payload.surface    = *surface;
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_CREATE_PRIMARY_SURFACE,
                            &payload);
    red_qxl_create_primary_surface_complete(qxl_state, surface);
}

static void red_qxl_create_primary_surface(QXLState *           qxl_state,
                                           uint32_t             surface_id,
                                           QXLDevSurfaceCreate *surface,
                                           int                  async,
                                           uint64_t             cookie)
...
static void red_qxl_reset_image_cache(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_RESET_IMAGE_CACHE, &payload);
}
...
static void red_qxl_reset_cursor(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_RESET_CURSOR, &payload);
}
...
static void red_qxl_destroy_surface_wait_sync(QXLState *qxl_state, uint32_t surface_id)
{
    RedWorkerMessageDestroySurfaceWait payload;

    payload.surface_id = surface_id;
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_DESTROY_SURFACE_WAIT,
                            &payload);
}

static void
red_qxl_destroy_surface_wait_async(QXLState *qxl_state, uint32_t surface_id, uint64_t cookie)
{
...
static void
red_qxl_destroy_surface_wait(QXLState *qxl_state, uint32_t surface_id, int async, uint64_t cookie)
{
    if (async) {
...
static void red_qxl_reset_memslots(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_RESET_MEMSLOTS, &payload);
}
...
static void red_qxl_wakeup(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_WAKEUP, &payload);
}
...
static void red_qxl_oom(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_OOM, &payload);
}
...
void red_qxl_start(QXLInstance *qxl)
{
    ...
    dispatcher_send_message(qxl->st->dispatcher, RED_WORKER_MESSAGE_START, &payload);
}
...
static void red_qxl_driver_unload(QXLState *qxl_state)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_DRIVER_UNLOAD, &payload);
}
...
void red_qxl_stop(QXLInstance *qxl)
{
    ...
    dispatcher_send_message(qxl->st->dispatcher, RED_WORKER_MESSAGE_STOP, &payload);
}
...
static void red_qxl_loadvm_commands(QXLState *qxl_state, struct QXLCommandExt *ext, uint32_t count)
{
    ...
    payload.count = count;
    payload.ext   = ext;
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_LOADVM_COMMANDS, &payload);
}

static void
qxl_worker_loadvm_commands(QXLWorker *qxl_worker, struct QXLCommandExt *ext, uint32_t count)
{
    QXLState *qxl_state = SPICE_CONTAINEROF(qxl_worker, QXLState, qxl_worker);
...
SPICE_GNUC_VISIBLE
void spice_qxl_wakeup(QXLInstance *instance) { red_qxl_wakeup(instance->st); }

SPICE_GNUC_VISIBLE
void spice_qxl_oom(QXLInstance *instance) { red_qxl_oom(instance->st); }

SPICE_GNUC_VISIBLE
void spice_qxl_start(QXLInstance *instance) { red_qxl_start(instance); }

SPICE_GNUC_VISIBLE
void spice_qxl_stop(QXLInstance *instance) { red_qxl_stop(instance); }

SPICE_GNUC_VISIBLE
void spice_qxl_update_area(QXLInstance *   instance,
                           uint32_t        surface_id,
                           struct QXLRect *area,
                           struct QXLRect *dirty_rects,
                           uint32_t        num_dirty_rects,
                           uint32_t        clear_dirty_region)
{
    red_qxl_update_area(instance->st, surface_id, area, dirty_rects, num_dirty_rects,
                        clear_dirty_region);
}
...
SPICE_GNUC_VISIBLE
void spice_qxl_reset_memslots(QXLInstance *instance) { red_qxl_reset_memslots(instance->st); }

SPICE_GNUC_VISIBLE
void spice_qxl_destroy_surfaces(QXLInstance *instance) { red_qxl_destroy_surfaces(instance->st); }
...
SPICE_GNUC_VISIBLE
void spice_qxl_create_primary_surface(QXLInstance *        instance,
                                      uint32_t             surface_id,
                                      QXLDevSurfaceCreate *surface)
{
...
SPICE_GNUC_VISIBLE
void spice_qxl_reset_image_cache(QXLInstance *instance) { red_qxl_reset_image_cache(instance->st); }

SPICE_GNUC_VISIBLE
void spice_qxl_reset_cursor(QXLInstance *instance) { red_qxl_reset_cursor(instance->st); }
...
SPICE_GNUC_VISIBLE
void spice_qxl_update_area_async(QXLInstance *instance,
                                 uint32_t     surface_id,
                                 QXLRect *    qxl_area,
                                 uint32_t     clear_dirty_region,
                                 uint64_t     cookie)
{
    red_qxl_update_area_async(instance->st, surface_id, qxl_area, clear_dirty_region, cookie);
}
...
SPICE_GNUC_VISIBLE
void spice_qxl_monitors_config_async(QXLInstance *instance,
                                     QXLPHYSICAL  monitors_config,
                                     int          group_id,
                                     uint64_t     cookie)
{
...
SPICE_GNUC_VISIBLE
void spice_qxl_driver_unload(QXLInstance *instance) { red_qxl_driver_unload(instance->st); }
                          ...
                          int          y_0_top)
{
    ...
    qxl_state->scanout =
        (SpiceMsgDisplayGlScanoutUnix){.flags          = y_0_top ? SPICE_GL_SCANOUT_FLAGS_Y0TOP : 0,
                                       .drm_dma_buf_fd = fd,
                                       .width          = width,
                                       .height         = height,
                                       .stride         = stride,
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_GL_SCANOUT, &payload);

    reds_update_client_mouse_allowed(qxl_state->reds);
}

SPICE_GNUC_VISIBLE
void spice_qxl_gl_draw_async(
    QXLInstance *qxl, uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint64_t cookie)
{
    QXLState *             qxl_state;
...
{
    QXLState *qxl_state;
    ClientCbs client_cursor_cbs = {
        NULL,
    };
    ClientCbs client_display_cbs = {
        NULL,
    };
    ...
    qxl_state->worker = red_worker_new(qxl, &client_cursor_cbs, &client_display_cbs);
...
void red_qxl_destroy(QXLInstance *qxl)
{
    ...
    dispatcher_send_message(qxl_state->dispatcher, RED_WORKER_MESSAGE_CLOSE_WORKER, &message);
...
Dispatcher *red_qxl_get_dispatcher(QXLInstance *qxl) { return qxl->st->dispatcher; }
...
{
    // try to get resolution when 3D enabled, since qemu did not create QXL
    // primary surface
...
void red_qxl_on_ic_change(QXLInstance *qxl, SpiceImageCompression ic)
{
    ...
    dispatcher_send_message(qxl->st->dispatcher, RED_WORKER_MESSAGE_SET_COMPRESSION, &payload);
}
...
void red_qxl_on_sv_change(QXLInstance *qxl, int sv)
{
    ...
    dispatcher_send_message(qxl->st->dispatcher, RED_WORKER_MESSAGE_SET_STREAMING_VIDEO, &payload);
}
...
void red_qxl_on_vc_change(QXLInstance *qxl, GArray *video_codecs)
{
    ...
    dispatcher_send_message(qxl->st->dispatcher, RED_WORKER_MESSAGE_SET_VIDEO_CODECS, &payload);
}
...
void red_qxl_set_mouse_mode(QXLInstance *qxl, uint32_t mode)
{
    ...
    payload.mode = mode;
    dispatcher_send_message(qxl->st->dispatcher, RED_WORKER_MESSAGE_SET_MOUSE_MODE, &payload);
}

RedsState *red_qxl_get_server(QXLState *qxl_state) { return qxl_state->reds; }
...
void red_qxl_update_area_complete(QXLInstance *   qxl,
                                  uint32_t        surface_id,
