...
static void stream_channel_client_class_init(StreamChannelClientClass *klass)
{
...
static void stream_channel_client_init(StreamChannelClient *client) { client->stream_id = -1; }

static void request_new_stream(StreamChannel *channel, StreamMsgStartStop *start)
{
    if (channel->start_cb) {
...
static void stream_channel_client_on_disconnect(RedChannelClient *rcc)
{
    ...
    StreamMsgStartStop stop = {
        0,
    };
    request_new_stream(channel, &stop);
}

static StreamChannelClient *stream_channel_client_new(StreamChannel *         channel,
                                                      RedClient *             client,
                                                      RedStream *             stream,
                                                      int                     mig_target,
                                                      RedChannelCapabilities *caps)
{
    ...
    rcc = g_initable_new(TYPE_STREAM_CHANNEL_CLIENT, NULL, NULL, "channel", channel, "client",
                         client, "stream", stream, "monitor-latency", FALSE, "caps", caps, NULL);

    return rcc;
}

static void fill_base(SpiceMarshaller *m, const StreamChannel *channel)
{
...
static void stream_channel_send_item(RedChannelClient *rcc, RedPipeItem *pipe_item)
{
    ...
    case RED_PIPE_ITEM_TYPE_SURFACE_CREATE: {
        ...
        SpiceMsgSurfaceCreate surface_create = {PRIMARY_SURFACE_ID, channel->width, channel->height,
...
static bool handle_message(RedChannelClient *rcc, uint16_t type, uint32_t size, void *msg)
{
    switch (type) {
...
{
    return g_object_new(TYPE_STREAM_CHANNEL, "spice-server", server, "core-interface",
                        reds_get_core_interface(server), "channel-type", SPICE_CHANNEL_DISPLAY,
                        // TODO this id should be after all qxl devices
                        "id", id, "migration-flags", 0, "handle-acks", TRUE, // TODO sure ??
                        NULL);
}
...
static uint8_t stream_channel_get_supported_codecs(StreamChannel *channel, uint8_t *out_codecs)
{
...
static void stream_channel_connect(RedChannel *            red_channel,
...
static void stream_channel_constructed(GObject *object)
{
...
static void stream_channel_class_init(StreamChannelClass *klass)
{
...
static void stream_channel_init(StreamChannel *channel)
{
    channel->stream_id = -1;
    channel->width     = 0;
    channel->height    = 0;
...
void stream_channel_change_format(StreamChannel *channel, const StreamMsgFormat *fmt)
{
...
static inline void
stream_channel_update_queue_stat(StreamChannel *channel, int32_t num_diff, int32_t size_diff)
{
                              ...
                              uint32_t       mm_time)
{
    ...
    red_pipe_item_init_full(&item->base, RED_PIPE_ITEM_TYPE_STREAM_DATA, data_item_free);
...
void stream_channel_register_queue_stat_cb(StreamChannel *                channel,
                                           stream_channel_queue_stat_proc cb,
                                           void *                         opaque)
{
    channel->queue_cb     = cb;
