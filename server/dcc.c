...
static void display_channel_client_get_property(GObject *   object,
                                                guint       property_id,
                                                GValue *    value,
                                                GParamSpec *pspec)
{
...
static void display_channel_client_set_property(GObject *     object,
                                                guint         property_id,
                                                const GValue *value,
                                                GParamSpec *  pspec)
{
...
static void display_channel_client_constructed(GObject *object)
{
    DisplayChannelClient *self = DISPLAY_CHANNEL_CLIENT(object);
...
static void display_channel_client_finalize(GObject *object)
{
    DisplayChannelClient *self = DISPLAY_CHANNEL_CLIENT(object);
...
static void display_channel_client_class_init(DisplayChannelClientClass *klass)
{
    ...
    g_object_class_install_property(
        object_class, PROP_IMAGE_COMPRESSION,
        g_param_spec_enum("image-compression", "image compression", "Image compression type",
                          SPICE_TYPE_SPICE_IMAGE_COMPRESSION_T, SPICE_IMAGE_COMPRESSION_INVALID,
                          G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_JPEG_STATE,
        g_param_spec_enum("jpeg-state", "jpeg state", "JPEG compression state",
                          SPICE_TYPE_SPICE_WAN_COMPRESSION_T, SPICE_WAN_COMPRESSION_INVALID,
                          G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        object_class, PROP_ZLIB_GLZ_STATE,
        g_param_spec_enum("zlib-glz-state", "zlib glz state", "zlib glz state",
                          SPICE_TYPE_SPICE_WAN_COMPRESSION_T, SPICE_WAN_COMPRESSION_INVALID,
                          G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static void display_channel_client_init(DisplayChannelClient *self)
{
    ...
    self->priv->send_data.free_list.res = g_malloc(
...
/*
 * Return: TRUE if wait_if_used == FALSE, or otherwise, if all of the pipe items
 * that
                                           ...
                                           int                   wait_if_used)
{
    ...
    for (l = red_channel_client_get_pipe(rcc)->head; l != NULL;) {
        ...
        if (depend_found) {
            ...
            return red_channel_client_wait_pipe_item_sent(rcc, item_pos, COMMON_CLIENT_TIMEOUT);
        }
    }
...
void dcc_create_surface(DisplayChannelClient *dcc, int surface_id)
{
    ...
    create  = red_surface_create_item_new(RED_CHANNEL(display), surface_id, surface->context.width,
                                         surface->context.height, surface->context.format, flags);
...
RedImageItem *dcc_add_surface_area_image(
    DisplayChannelClient *dcc, int surface_id, SpiceRect *area, GList *pipe_item_pos, int can_lossy)
{
    ...
    item->image_format = spice_bitmap_from_surface_type(surface->context.format);
    item->image_flags  = 0;
    ...
    if (!is_primary_surface(display, surface_id) && item->image_format == SPICE_BITMAP_FMT_32BIT &&
...

{
    ...
    dcc = g_initable_new(TYPE_DISPLAY_CHANNEL_CLIENT, NULL, NULL, "channel", display, "client",
                         client, "stream", stream, "monitor-latency", TRUE, "caps", caps,
                         "image-compression", image_compression, "jpeg-state", jpeg_state,
                         "zlib-glz-state", zlib_glz_state, NULL);
...
static RedSurfaceDestroyItem *red_surface_destroy_item_new(RedChannel *channel, uint32_t surface_id)
{
...
{
    return !bitmap_fmt_is_plt(bitmap->format) && bitmap->x >= MIN_DIMENSION_TO_QUIC &&
           bitmap->y >= MIN_DIMENSION_TO_QUIC;
}
/**
 * lz doesn't handle:
 *       (1) bitmaps with strides that are larger than the width of the image in
 * bytes
 *       (2) unstable bitmaps
 */
static bool can_lz_compress(SpiceBitmap *bitmap)
{
    return !bitmap_has_extra_stride(bitmap) && !(bitmap->data->flags & SPICE_CHUNKS_FLAGS_UNSTABLE);
}
                                                        ...
                                                        Drawable *            drawable)
{
    ...
    if (preferred_compression == SPICE_IMAGE_COMPRESSION_AUTO_GLZ ||
        ...
        if (can_quic_compress(bitmap)) {
            if (drawable == NULL || drawable->copy_bitmap_graduality == BITMAP_GRADUAL_INVALID) {
                       ...
                       compress_send_data_t *o_comp_data)
{
    ...
    case SPICE_IMAGE_COMPRESSION_GLZ:
        success = image_encoders_compress_glz(
            &dcc->priv->encoders, dest, src, drawable->red_drawable, &drawable->glz_retention,
            o_comp_data, display_channel->priv->enable_zlib_glz_wrap);
        if (success) {
            break;
        }
        goto lz_compress;
    ...
    if (!success) {
        ...
        stat_compress_add(&display_channel->priv->encoder_shared_data.off_stat, start_time,
...
void dcc_palette_cache_palette(DisplayChannelClient *dcc, SpicePalette *palette, uint8_t *flags)
{
    if (palette == NULL) {
        return;
    }
...
static void
dcc_push_release(DisplayChannelClient *dcc, uint8_t type, uint64_t id, uint64_t *sync_data)
{
...
bool dcc_pixmap_cache_unlocked_add(DisplayChannelClient *dcc, uint64_t id, uint32_t size, int lossy)
{
...
static bool dcc_handle_init(DisplayChannelClient *dcc, SpiceMsgcDisplayInit *init)
{
    ...
    dcc->priv->pixmap_cache =
        pixmap_cache_get(client, init->pixmap_cache_id, init->pixmap_cache_size);
    spice_return_val_if_fail(dcc->priv->pixmap_cache, FALSE);

    success = image_encoders_get_glz_dictionary(
        &dcc->priv->encoders, client, init->glz_dictionary_id, init->glz_dictionary_window_size);
                                     ...
                                     SpiceMsgcDisplayStreamReport *report)
{
    ...
    if (report->stream_id >= NUM_STREAMS) {
        spice_warning("stream_report: invalid stream id %u", report->stream_id);
        return FALSE;
    }
    ...
    if (report->num_frames == 0 && report->num_drops == UINT32_MAX) {
        spice_warning("stream_report: the client does not support stream %u", report->stream_id);
    ...
    agent->video_encoder->client_stream_report(
        agent->video_encoder, report->num_frames, report->num_drops, report->start_frame_mm_time,
        report->end_frame_mm_time, report->last_frame_delay, report->audio_delay);
    return TRUE;
}
                                                   ...
                                                   gpointer      user_data)
{
    ...
    return (g_array_index(client_pref, gint, a->type) - g_array_index(client_pref, gint, b->type));
}
...
bool dcc_handle_message(RedChannelClient *rcc, uint16_t type, uint32_t size, void *msg)
{
    ...
    case SPICE_MSGC_DISPLAY_PREFERRED_COMPRESSION:
        return dcc_handle_preferred_compression(dcc, (SpiceMsgcDisplayPreferredCompression *)msg);
    ...
    case SPICE_MSGC_DISPLAY_PREFERRED_VIDEO_CODEC_TYPE:
        return dcc_handle_preferred_video_codec_type(
            dcc, (SpiceMsgcDisplayPreferredVideoCodecType *)msg);
    default:
...
{
    return image_encoders_restore_glz_dictionary(
        &dcc->priv->encoders, red_channel_client_get_client(RED_CHANNEL_CLIENT(dcc)),
        migrate->glz_dict_id, &migrate->glz_dict_data);
}
...
bool dcc_handle_migrate_data(DisplayChannelClient *dcc, uint32_t size, void *message)
{
    ...
    spice_return_val_if_fail(size >= (sizeof(*migrate_data) + sizeof(SpiceMigrateDataHeader)),
                             FALSE);
    spice_return_val_if_fail(migration_protocol_validate_header(header,
     ...
     * pixmap_cache_reset */
    dcc->priv->pixmap_cache = pixmap_cache_get(
        red_channel_client_get_client(RED_CHANNEL_CLIENT(dcc)), migrate_data->pixmap_cache_id, -1);
    spice_return_val_if_fail(dcc->priv->pixmap_cache, FALSE);

    pthread_mutex_lock(&dcc->priv->pixmap_cache->lock);
    for (i = 0; i < MAX_CACHE_CLIENTS; i++) {
        dcc->priv->pixmap_cache->sync[i] =
            MAX(dcc->priv->pixmap_cache->sync[i], migrate_data->pixmap_cache_clients[i]);
    }
...
ImageEncoders *dcc_get_encoders(DisplayChannelClient *dcc) { return &dcc->priv->encoders; }
...
gboolean dcc_is_low_bandwidth(DisplayChannelClient *dcc) { return dcc->is_low_bandwidth; }
