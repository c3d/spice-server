...
static void dest_mgr_init_destination(j_compress_ptr cinfo)
{
    ...
    if (enc->dest_mgr.free_in_buffer == 0) {
        enc->dest_mgr.free_in_buffer =
            enc->usr->more_space(enc->usr, &enc->dest_mgr.next_output_byte);
...
{
    JpegEncoder *enc             = (JpegEncoder *)cinfo->client_data;
    enc->dest_mgr.free_in_buffer = enc->usr->more_space(enc->usr, &enc->dest_mgr.next_output_byte);
...
static void convert_RGB24_to_RGB24(void *line, int width, uint8_t **out_line) { *out_line = line; }
...
int jpeg_encode(JpegEncoderContext * jpeg,
                int                  quality,
                JpegEncoderImageType type,
                int                  width,
                int                  height,
