...
static const char *spice_audio_data_mode_to_string(gint mode)
{
    static const char *const str[] = {
            [SPICE_AUDIO_DATA_MODE_INVALID] = "invalid", [SPICE_AUDIO_DATA_MODE_RAW] = "raw",
            [SPICE_AUDIO_DATA_MODE_CELT_0_5_1] = "celt", [SPICE_AUDIO_DATA_MODE_OPUS] = "opus",
    };
...
static bool snd_send_volume(SndChannelClient *client, uint32_t cap, int msg)
{
    ...
    vol = alloca(sizeof(SpiceMsgAudioVolume) + st->volume_nchannels * sizeof(uint16_t));
...
static void snd_persistent_pipe_item_free(struct RedPipeItem *item)
{
    ...
    red_pipe_item_init_full(item, RED_PIPE_ITEM_PERSISTENT, snd_persistent_pipe_item_free);
...
static void playback_channel_send_item(RedChannelClient *rcc, G_GNUC_UNUSED RedPipeItem *item)
{
    ...
    client->command &= SND_PLAYBACK_MODE_MASK | SND_PLAYBACK_PCM_MASK | SND_CTRL_MASK |
                       SND_VOLUME_MUTE_MASK | SND_MIGRATE_MASK | SND_PLAYBACK_LATENCY_MASK;
...
#ifdef SO_PRIORITY
    priority = 6;
    if (setsockopt(stream->socket, SOL_SOCKET, SO_PRIORITY, (void *)&priority, sizeof(priority)) ==
        -1) {
        if (errno != ENOTSUP) {
...
static void snd_channel_client_release_recv_buf(RedChannelClient *rcc,
                                                uint16_t          type,
                                                uint32_t          size,
                                                uint8_t *         msg)
{
...
static void snd_channel_set_volume(SndChannel *channel, uint8_t nchannels, uint16_t *volume)
{
    SpiceVolumeState *st     = &channel->volume;
...
spice_server_playback_set_volume(SpicePlaybackInstance *sin, uint8_t nchannels, uint16_t *volume)
{
...
SPICE_GNUC_VISIBLE void spice_server_playback_put_samples(SpicePlaybackInstance *sin,
                                                          uint32_t *             samples)
{
    ...
    if (!playback_client ||
        snd_channel_get_client(&sin->st->channel) != SND_CHANNEL_CLIENT(playback_client)) {
...
static int snd_desired_audio_mode(bool playback_compression,
                                  int  frequency,
...
static void playback_channel_client_finalize(GObject *object)
{
    int                    i;
...
static void playback_channel_client_constructed(GObject *object)
{
    PlaybackChannelClient *playback_client = PLAYBACK_CHANNEL_CLIENT(object);
    RedChannelClient *     rcc             = RED_CHANNEL_CLIENT(playback_client);
    ...
    bool client_can_celt = red_channel_client_test_remote_cap(rcc, SPICE_PLAYBACK_CAP_CELT_0_5_1);
    bool client_can_opus = red_channel_client_test_remote_cap(rcc, SPICE_PLAYBACK_CAP_OPUS);
...
static void snd_set_peer(RedChannel *            red_channel,
                         RedClient *             client,
                         RedStream *             stream,
                         ...
                         GType                   type)
{
    ...
    snd_client = g_initable_new(type, NULL, NULL, "channel", channel, "client", client, "stream",
                                stream, "caps", caps, NULL);
    g_warn_if_fail(snd_client != NULL);
}

static void snd_set_playback_peer(RedChannel *            red_channel,
                                  RedClient *             client,
                                  RedStream *             stream,
                                  G_GNUC_UNUSED int       migration,
                                  RedChannelCapabilities *caps)
{
    snd_set_peer(red_channel, client, stream, caps, TYPE_PLAYBACK_CHANNEL_CLIENT);
}
...
SPICE_GNUC_VISIBLE void
spice_server_record_set_volume(SpiceRecordInstance *sin, uint8_t nchannels, uint16_t *volume)
{
    snd_channel_set_volume(&sin->st->channel, nchannels, volume);
}
...
static uint32_t snd_get_best_rate(SndChannelClient *client, uint32_t cap_opus)
{
    ...
    if (client_can_opus &&
        snd_codec_is_capable(SPICE_AUDIO_DATA_MODE_OPUS, SND_CODEC_ANY_FREQUENCY))
...
SPICE_GNUC_VISIBLE void spice_server_set_playback_rate(SpicePlaybackInstance *sin,
                                                       uint32_t               frequency)
{
...
static void record_channel_client_finalize(GObject *object)
{
...
static void record_channel_client_constructed(GObject *object)
{
    RecordChannelClient *record_client = RECORD_CHANNEL_CLIENT(object);
...
static void snd_set_record_peer(RedChannel *            red_channel,
                                RedClient *             client,
                                RedStream *             stream,
                                G_GNUC_UNUSED int       migration,
                                RedChannelCapabilities *caps)
{
    snd_set_peer(red_channel, client, stream, caps, TYPE_RECORD_CHANNEL_CLIENT);
}
...
static void snd_channel_init(SndChannel *self)
{
    self->frequency = SND_CODEC_CELT_PLAYBACK_FREQ; /* Default to the legacy rate */
}

static void snd_channel_finalize(GObject *object)
{
...
static void snd_channel_class_init(SndChannelClass *klass)
{
...
static void playback_channel_init(PlaybackChannel *self) {}

static void playback_channel_constructed(GObject *object)
{
...
static void playback_channel_class_init(PlaybackChannelClass *klass)
{
    GObjectClass *   object_class  = G_OBJECT_CLASS(klass);
...
{
    sin->st = g_object_new(TYPE_PLAYBACK_CHANNEL, "spice-server", reds, "core-interface",
                           reds_get_core_interface(reds), "channel-type", SPICE_CHANNEL_PLAYBACK,
                           "id", 0, NULL);
}

static void record_channel_init(RecordChannel *self) {}

static void record_channel_constructed(GObject *object)
{
    ClientCbs client_cbs = {
        NULL,
    };
...
static void record_channel_class_init(RecordChannelClass *klass)
{
    GObjectClass *   object_class  = G_OBJECT_CLASS(klass);
    RedChannelClass *channel_class = RED_CHANNEL_CLASS(klass);
...
{
    sin->st = g_object_new(TYPE_RECORD_CHANNEL, "spice-server", reds, "core-interface",
                           reds_get_core_interface(reds), "channel-type", SPICE_CHANNEL_RECORD,
                           "id", 0, NULL);
}
...
void snd_detach_playback(SpicePlaybackInstance *sin) { snd_detach_common(&sin->st->channel); }

void snd_detach_record(SpiceRecordInstance *sin) { snd_detach_common(&sin->st->channel); }
...
void snd_set_playback_compression(bool on)
{
    ...
    for (l = snd_channels; l != NULL; l = l->next) {
        ...
        if (type == SPICE_CHANNEL_PLAYBACK && client) {
            ...
            bool                   client_can_celt =
                red_channel_client_test_remote_cap(rcc, SPICE_PLAYBACK_CAP_CELT_0_5_1);
            bool client_can_opus = red_channel_client_test_remote_cap(rcc, SPICE_PLAYBACK_CAP_OPUS);
            int  desired_mode =
                snd_desired_audio_mode(on, now->frequency, client_can_celt, client_can_opus);
...
static void snd_channel_client_class_init(SndChannelClientClass *klass)
{
...
static void snd_channel_client_init(SndChannelClient *self) {}

static void playback_channel_client_class_init(PlaybackChannelClientClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
...
static void playback_channel_client_init(PlaybackChannelClient *playback)
{
    playback->mode = SPICE_AUDIO_DATA_MODE_RAW;
    snd_playback_alloc_frames(playback);
}

static void record_channel_client_class_init(RecordChannelClientClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
...
static void record_channel_client_init(RecordChannelClient *record)
{
