...
static bool glz_dictionary_window_create(SharedDictionary *dict, uint32_t size)
{
    ...
    dict->window.segs       = (WindowImageSegment *)(dict->cur_usr->malloc(
        dict->cur_usr, sizeof(WindowImageSegment) * INIT_IMAGE_SEGS_NUM));
    ...
    dict->window.encoders_heads =
        (uint32_t *)dict->cur_usr->malloc(dict->cur_usr, sizeof(uint32_t) * dict->max_encoders);
...
GlzEncDictContext *
glz_enc_dictionary_create(uint32_t size, uint32_t max_encoders, GlzEncoderUsrContext *usr)
{
    SharedDictionary *dict;

    if (!(dict = (SharedDictionary *)usr->malloc(usr, sizeof(SharedDictionary)))) {
        return NULL;
    }
...
/***********************************************************************************
 Mutators of the window. Should be called by the encoder before and after
 encoding.
 ***********************************************************************************/
...
static void __glz_dictionary_window_segs_realloc(SharedDictionary *dict)
{
    ...
    new_segs = (WindowImageSegment *)dict->cur_usr->malloc(dict->cur_usr,
                                                           sizeof(WindowImageSegment) * new_quota);

    if (!new_segs) {
        dict->cur_usr->error(dict->cur_usr, "realloc of dictionary window failed\n");
    }

    memcpy(new_segs, dict->window.segs, sizeof(WindowImageSegment) * dict->window.segs_quota);
...
static WindowImage *__glz_dictionary_window_alloc_image(SharedDictionary *dict)
{
    ...
    if (dict->window.free_images) {
        ...
        if (!(ret = (WindowImage *)dict->cur_usr->malloc(dict->cur_usr, sizeof(*ret)))) {
            return NULL;
        }
    }
...
/* moves image to free list and "kill" it. Calls the free_image callback if was
 * alive. */
...
/* moves all the segments that were associated with the images to the free
 * segments */
...
/* Returns the logical head of the window after we add an image with the give
   size to its tail.
   Returns NULL when the window is empty, of when we have to empty the window in
   order
...
static WindowImage *glz_dictionary_window_get_new_head(SharedDictionary *dict, int new_image_size)
{
    ...
    // used_segs_head is the latest logical head (the physical head may preceed
    // it)
...
/* remove from the window (and free relevant data) the images between the oldest
   physical head
   (inclusive) and the end_image (exclusive). If end_image is NULL, empties the
   window*/
static void glz_dictionary_window_remove_head(SharedDictionary *dict,
                                              uint32_t          encoder_id,
...
static uint32_t glz_dictionary_window_alloc_image_seg(SharedDictionary *dict,
                                                      WindowImage *     image,
                                                      int               size,
                                                      int               stride,
                                                      uint8_t *         lines,
...
static WindowImage *glz_dictionary_window_add_image(SharedDictionary *  dict,
                                                    ...
                                                    GlzUsrImageContext *usr_image_context)
{
    ...
    for (row = 0;;) {
        seg_id = glz_dictionary_window_alloc_image_seg(
            dict, image, image_size * num_lines / image_height, image_stride, lines, num_lines);
        if (row == 0) {
    ...
    if (dict->window.used_segs_tail == NULL_IMAGE_SEG_ID) {
        ...
        // For the other thread that may read 'next' of the old tail,
        // NULL_IMAGE_SEG_ID
        // is equivalent to a segment with an image id that is different
        // from the image id of the tail, so we don't need to further protect this
        // field.
...
WindowImage *glz_dictionary_pre_encode(uint32_t              encoder_id,
                                       GlzEncoderUsrContext *usr,
                                       SharedDictionary *    dict,
                                       LzImageType           image_type,
                                       int                   image_width,
...
void glz_dictionary_post_encode(uint32_t              encoder_id,
                                GlzEncoderUsrContext *usr,
                                SharedDictionary *    dict)
{
    uint32_t i;
