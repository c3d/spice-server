...
static SpiceTimer *
adapter_timer_add(const SpiceCoreInterfaceInternal *iface, SpiceTimerFunc func, void *opaque)
{
    return iface->public_interface->timer_add(func, opaque);
}

static void
adapter_timer_start(const SpiceCoreInterfaceInternal *iface, SpiceTimer *timer, uint32_t ms)
{
...
#define RED_CHAR_DEVICE_VDIPORT_PRIVATE(o)                                                         \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), RED_TYPE_CHAR_DEVICE_VDIPORT, RedCharDeviceVDIPortPrivate))
...
void stat_init_node(
    RedStatNode *node, SpiceServer *reds, const RedStatNode *parent, const char *name, int visible)
{
...
void stat_init_counter(RedStatCounter *   counter,
                       SpiceServer *      reds,
                       const RedStatNode *parent,
                       const char *       name,
                       int                visible)
{
    StatNodeRef parent_ref = parent ? parent->ref : INVALID_STAT_REF;
    counter->counter       = stat_file_add_counter(reds->stat_file, parent_ref, name, visible);
}
...
static void reds_mig_cleanup(RedsState *reds)
{
    if (reds->mig_inprogress) {
        ...
        if (reds->mig_wait_connect || reds->mig_wait_disconnect) {
            ...
            sif =
                SPICE_CONTAINEROF(reds->migration_interface->base.sif, SpiceMigrateInterface, base);
...
static void reds_reset_vdp(RedsState *reds)
{
    ...
    agent_msg_filter_init(&dev->priv->read_filter, reds->config->agent_copypaste,
                          reds->config->agent_file_xfer, reds_use_client_monitors_config(reds),
                          TRUE);
     ...
     *  in order to be backward compatible with the client, we need to track the
     * tokens
...
static void reds_disconnect(RedsState *reds)
{
    ...
    spice_debug("trace");
    GLIST_FOREACH(reds->clients, RedClient, client) { reds_client_disconnect(reds, client); }
    reds_mig_cleanup(reds);
}
...
SpiceMouseMode reds_get_mouse_mode(RedsState *reds) { return reds->mouse_mode; }
...
static void reds_set_mouse_mode(RedsState *reds, SpiceMouseMode mode)
{
    ...
    FOREACH_QXL_INSTANCE(reds, qxl) { red_qxl_set_mouse_mode(qxl, mode); }

    main_channel_push_mouse_mode(reds->main_channel, reds->mouse_mode,
                                 reds->is_client_mouse_allowed);
}

gboolean reds_config_get_agent_mouse(const RedsState *reds) { return reds->config->agent_mouse; }
...
{
    // TODO: agent is broken with multiple clients. also need to figure out what
    // to do when
...
*/
static AgentMsgFilterResult vdi_port_read_buf_process(RedCharDeviceVDIPort *dev, RedVDIReadBuf *buf)
{
...
static RedVDIReadBuf *vdi_read_buf_new(RedCharDeviceVDIPort *dev)
{
     ...
     */
    red_pipe_item_init_full(&buf->base, -1, vdi_port_read_buf_free);
    buf->dev = dev;
    buf->len = 0;
    return buf;
}
...
static void
agent_adjust_capabilities(VDAgentMessage *message, bool clipboard_enabled, bool xfer_enabled)
{
    ...
    while (reds->vdagent) {
        ...
        case VDI_PORT_READ_STATE_GET_BUFF: {
            ...
            dev->priv->receive_len =
                MIN(dev->priv->message_receive_len, sizeof(dev->priv->current_read_buf->data));
 ...
 */
static void vdi_port_send_msg_to_client(RedCharDevice *self, RedPipeItem *msg, RedClient *client)
{
    RedVDIReadBuf *agent_data_buf = (RedVDIReadBuf *)msg;

    red_pipe_item_ref(msg);
    main_channel_client_push_agent_data(red_client_get_main(client), agent_data_buf->data,
                                        agent_data_buf->len, vdi_port_read_buf_release,
                                        agent_data_buf);
}

static void vdi_port_send_tokens_to_client(RedCharDevice *self, RedClient *client, uint32_t tokens)
{
    main_channel_client_push_agent_tokens(red_client_get_main(client), tokens);
}
...
static void vdi_port_remove_client(RedCharDevice *self, RedClient *client)
{
...
int reds_has_vdagent(RedsState *reds) { return !!reds->vdagent; }
...
void reds_handle_agent_mouse_event(RedsState *reds, const VDAgentMouseState *mouse_state)
{
    ...
    total_msg_size = sizeof(VDIChunkHeader) + sizeof(VDAgentMessage) + sizeof(VDAgentMouseState);
    char_dev_buf =
        red_char_device_write_buffer_get(RED_CHAR_DEVICE(reds->agent_dev), NULL, total_msg_size);
...
static void reds_fill_channels(RedsState *reds, SpiceMsgChannels *channels_info)
{
    ...
    {
        uint32_t type, id;
        if (g_list_length(reds->clients) > 1 && !channel_supports_multiple_clients(channel)) {
            continue;
        }
...
SpiceMsgChannels *reds_msg_channels_new(RedsState *reds)
{
    ...
    channels_info = (SpiceMsgChannels *)g_malloc(
        sizeof(SpiceMsgChannels) + g_list_length(reds->channels) * sizeof(SpiceChannelId));
...
void reds_on_main_agent_start(RedsState *reds, MainChannelClient *mcc, uint32_t num_tokens)
{
    ...
    if (!red_char_device_client_exists(dev_state, client)) {
        ...
        client_added = red_char_device_client_add(
            dev_state, client, TRUE, /* flow control */
            REDS_VDI_PORT_NUM_RECEIVE_BUFFS, REDS_AGENT_WINDOW_SIZE, num_tokens,
        ...
        red_char_device_send_to_client_tokens_set(dev_state, client, num_tokens);
    }
...
void reds_on_main_agent_tokens(RedsState *reds, MainChannelClient *mcc, uint32_t num_tokens)
{
    ...
    red_char_device_send_to_client_tokens_add(reds->vdagent->st, client, num_tokens);
}
...
uint8_t *reds_get_agent_data_buffer(RedsState *reds, MainChannelClient *mcc, size_t size)
{
    ...
    if (!dev->priv->client_agent_started) {
        /*
         * agent got disconnected, and possibly got reconnected, but we still can
         * receive
         ...
         * In such case, we will receive and discard the msgs (reds_reset_vdp takes
         * care
    ...
    dev->priv->recv_from_client_buf = red_char_device_write_buffer_get(
        RED_CHAR_DEVICE(dev), client, size + sizeof(VDIChunkHeader));
...
void reds_release_agent_data_buffer(RedsState *reds, uint8_t *buf)
{
    ...
    /* if we pushed the buffer the buffer is attached to the channel so don't free
     * it */
                                               ...
                                               size_t             size)
{
    ...
    max_monitors = (msg_header->size - sizeof(VDAgentMonitorsConfig)) / sizeof(VDAgentMonConfig);
...
void reds_on_main_agent_data(RedsState *        reds,
                             MainChannelClient *mcc,
                             const void *       message,
                             size_t             size)
{
    ...
    res = agent_msg_filter_process_data(&reds->agent_dev->priv->write_filter, message, size);
    switch (res) {
    case AGENT_MSG_FILTER_OK:
        break;
    case AGENT_MSG_FILTER_DISCARD:
    ...
    spice_assert(message ==
                 reds->agent_dev->priv->recv_from_client_buf->buf + sizeof(VDIChunkHeader));
    ...
    if (agent_dev->priv->read_filter.msg_data_to_read ||
        ...
        case AGENT_MSG_FILTER_OK:
            agent_adjust_capabilities((VDAgentMessage *)read_buf->data,
                                      reds->config->agent_copypaste, reds->config->agent_file_xfer);
            main_channel_client_push_agent_data(mcc, read_buf->data, read_buf->len,
...
void reds_marshall_migrate_data(RedsState *reds, SpiceMarshaller *m)
{
    ...
    if (!reds->vdagent) {
        ...
        null_agent_mig_data = spice_marshaller_reserve_space(
            m, sizeof(SpiceMigrateDataMain) - sizeof(SpiceMigrateDataCharDevice));
        memset(null_agent_mig_data, 0,
    ...
    spice_marshaller_add(m, (uint8_t *)&mig_data.agent2client.chunk_header, sizeof(VDIChunkHeader));
    ...
    spice_debug("from agent filter: discard all %d, wait_msg %u, msg_filter_result %d",
                ...
                agent_dev->priv->read_filter.msg_data_to_read, agent_dev->priv->read_filter.result);
...
static int reds_agent_state_restore(RedsState *reds, SpiceMigrateDataMain *mig_data)
{
    ...
    if (!mig_data->agent2client.msg_header_done) {
        ...
        if (!chunk_header_remaining) {
            ...
            memcpy(agent_dev->priv->current_read_buf->data, partial_msg_header,
                   mig_data->agent2client.msg_header_partial_len);
            agent_dev->priv->receive_pos = agent_dev->priv->current_read_buf->data +
                                           mig_data->agent2client.msg_header_partial_len;
            ...
            agent_dev->priv->current_read_buf->len =
                agent_dev->priv->receive_len + mig_data->agent2client.msg_header_partial_len;
            agent_dev->priv->message_receive_len -= agent_dev->priv->receive_len;
        } else {
 ...
 */
bool reds_handle_migrate_data(RedsState *           reds,
                              MainChannelClient *   mcc,
                              SpiceMigrateDataMain *mig_data,
                              uint32_t              size)
{
    ...
    if (mig_data->agent_base.connected) {
        if (agent_dev->priv->agent_attached) { // agent was attached before
                                               // migration data has arrived
            if (!reds->vdagent) {
                ...
                if (agent_dev->priv->plug_generation > 1) {
                    /* red_char_device_state_reset takes care of not making the device
                     * wait for migration data */
                    spice_debug("agent has been detached and reattached before receiving "
                                "migration data");
                    main_channel_push_agent_disconnected(reds->main_channel);
...
static bool reds_send_link_ack(RedsState *reds, RedLinkInfo *link)
{
    ...
    channel = reds_find_channel(reds, link->link_mess->channel_type, link->link_mess->channel_id);
    if (!channel) {
    ...
    if (!reds->config->sasl_enabled ||
        ...
        if (RSA_generate_key_ex(link->tiTicketing.rsa, SPICE_TICKET_KEY_PAIR_LENGTH,
                                link->tiTicketing.bn, NULL) != 1) {
            spice_warning("Failed to generate %d bits RSA key: %s", SPICE_TICKET_KEY_PAIR_LENGTH,
...
static void reds_info_new_channel(RedLinkInfo *link, int connection_id)
{
    spice_debug("channel %d:%d, connected successfully, over %s link",
                link->link_mess->channel_type, link->link_mess->channel_id,
                red_stream_is_ssl(link->stream) ? "Secure" : "Non Secure");
    /* add info + send event */
    red_stream_set_channel(link->stream, connection_id, link->link_mess->channel_type,
...
static void red_channel_capabilities_init_from_link_message(RedChannelCapabilities *caps,
                                                            const SpiceLinkMess *   link_mess)
{
    ...
    if (caps->num_common_caps) {
        caps->common_caps = g_memdup(raw_caps, link_mess->num_common_caps * sizeof(uint32_t));
    }
...
static void reds_handle_main_link(RedsState *reds, RedLinkInfo *link)
{
    ...
    if (link_mess->connection_id == 0) {
        ...
        // connection id the migration src had (use vmstate to store the connection
        // id)
        reds_send_link_result(link, SPICE_LINK_ERR_OK);
    ...
    mcc = main_channel_link(reds->main_channel, client, stream, connection_id, mig_target, &caps);
    ...
    if (reds->vdagent) {
        ...
        agent_msg_filter_config(&reds->agent_dev->priv->read_filter, reds->config->agent_copypaste,
                                reds->config->agent_file_xfer,
    ...
    if (!mig_target) {
        main_channel_client_push_init(mcc, g_list_length(reds->qxl_instances), reds->mouse_mode,
                                      reds->is_client_mouse_allowed,
                                      reds_get_mm_time() - MM_TIME_DELTA, reds_qxl_ram_size(reds));
...
#define RED_MOUSE_STATE_TO_LOCAL(state)                                                            \
    ((state & SPICE_MOUSE_BUTTON_MASK_LEFT) | ((state & SPICE_MOUSE_BUTTON_MASK_MIDDLE) << 1) |    \
...
static void reds_channel_do_link(RedChannel *   channel,
                                 RedClient *    client,
                                 SpiceLinkMess *link_msg,
                                 RedStream *    stream)
{
    ...
    red_channel_connect(channel, client, stream, red_client_during_migrate_at_target(client),
                        &caps);
    red_channel_capabilities_reset(&caps);
}
...
static bool reds_link_mig_target_channels(RedsState *reds, RedClient *client)
{
    ...
    for (item = mig_client->pending_links; item != NULL; item = item->next) {
        ...
        if (!channel) {
            spice_warning("client %p channel (%d, %d) (type, id) wasn't found", client,
                          mig_link->link_msg->channel_type, mig_link->link_msg->channel_id);
            continue;
        }
...
void reds_on_client_semi_seamless_migrate_complete(RedsState *reds, RedClient *client)
{
    ...
    main_channel_client_push_init(mcc, g_list_length(reds->qxl_instances), reds->mouse_mode,
                                  reds->is_client_mouse_allowed, reds_get_mm_time() - MM_TIME_DELTA,
...
static void reds_handle_other_links(RedsState *reds, RedLinkInfo *link)
{
    ...
    link_mess = link->link_mess;
    if (reds->main_channel) {
        client = main_channel_get_client_by_link_id(reds->main_channel, link_mess->connection_id);
    }

    // TODO: MC: broke migration (at least for the dont-drop-connection kind).
    // On migration we should get a connection_id to expect (must be a security
    // measure)
    ...
    // TODO: MC: be less lenient. Tally connections from same connection_id (by
    // same client).
    if (!(channel = reds_find_channel(reds, link_mess->channel_type, link_mess->channel_id))) {
     ...
     * scratch we don't mind if we skip some messages in between the src session
     * end and
     * dst session start.
     * In seamless migration, in order to keep the continuousness of the session,
     * and
     * in order not to lose any data, we activate the target channels before
     * migration completes, as soon as we receive
     * SPICE_MSGC_MAIN_MIGRATE_DST_DO_SEAMLESS.
     * If a channel connects before receiving
     * SPICE_MSGC_MAIN_MIGRATE_DST_DO_SEAMLESS,
...
static void reds_handle_ticket(void *opaque)
{
    ...
    password_size = RSA_private_decrypt(
        link->tiTicketing.rsa_size, link->tiTicketing.encrypted_ticket.encrypted_data,
...
static int reds_security_check(RedLinkInfo *link)
{
    ...
    uint32_t security =
        security_option ? security_option->options : reds->config->default_channel_security;
...
static void reds_handle_read_link_done(void *opaque)
{
    ...
    if (!auth_selection) {
        ...
        red_stream_async_read(link->stream, (uint8_t *)&link->auth_mechanism,
...
static void reds_handle_read_header_done(void *opaque)
{
    ...
    red_stream_async_read(link->stream, (uint8_t *)link->link_mess, header->size,
...
static void reds_handle_read_magic_done(void *opaque)
{
    ...
    red_stream_async_read(link->stream, ((uint8_t *)&link->link_header) + sizeof(header->magic),
...
static void reds_handle_new_link(RedLinkInfo *link)
{
    ...
    red_stream_async_read(link->stream, (uint8_t *)&link->link_header,
...
static void reds_handle_ssl_accept(int fd, int event, void *data)
{
    ...
    case RED_STREAM_SSL_STATUS_WAIT_FOR_READ:
        reds_core_watch_update_mask(reds, link->stream->watch, SPICE_WATCH_EVENT_READ);
        return;
    case RED_STREAM_SSL_STATUS_WAIT_FOR_WRITE:
        reds_core_watch_update_mask(reds, link->stream->watch, SPICE_WATCH_EVENT_WRITE);
        return;
...
static RedLinkInfo *reds_init_client_ssl_connection(RedsState *reds, int socket)
{
    ...
    case RED_STREAM_SSL_STATUS_WAIT_FOR_READ:
        link->stream->watch = reds_core_watch_add(
            reds, link->stream->socket, SPICE_WATCH_EVENT_READ, reds_handle_ssl_accept, link);
        break;
    case RED_STREAM_SSL_STATUS_WAIT_FOR_WRITE:
        link->stream->watch = reds_core_watch_add(
            reds, link->stream->socket, SPICE_WATCH_EVENT_WRITE, reds_handle_ssl_accept, link);
        break;
    }
    return link;
...
static int reds_init_socket(const char *addr, int portnr, int family)
{
    ...
    if (rc != 0) {
        spice_warning("getaddrinfo(%s,%s): %s", addr, port, gai_strerror(rc));
        return -1;
    }
    ...
    for (e = res; e != NULL; e = e->ai_next) {
        ...
        if (bind(slisten, e->ai_addr, e->ai_addrlen) == 0) {
            ...
            char uport[33];
            rc = getnameinfo((struct sockaddr *)e->ai_addr, e->ai_addrlen, uaddr, INET6_ADDRSTRLEN,
                             uport, 32, NI_NUMERICHOST | NI_NUMERICSERV);
            if (rc == 0) {
...
void reds_set_client_mm_time_latency(RedsState *reds, RedClient *client, uint32_t latency)
{
    ...
    if (reds->mm_time_enabled) {
        ...
        if (latency > reds->mm_time_latency) {
            ...
            spice_debug("new latency %u is smaller than existing %u", latency,
...
static int reds_init_net(RedsState *reds)
{
    if (reds->config->spice_port != -1 || reds->config->spice_family == AF_UNIX) {
        reds->listen_socket = reds_init_socket(reds->config->spice_addr, reds->config->spice_port,
                                               reds->config->spice_family);
        if (-1 == reds->listen_socket) {
            return -1;
        }
        reds->listen_watch = reds_core_watch_add(reds, reds->listen_socket, SPICE_WATCH_EVENT_READ,
                                                 reds_accept, reds);
    ...
    if (reds->config->spice_secure_port != -1) {
        reds->secure_listen_socket = reds_init_socket(
            reds->config->spice_addr, reds->config->spice_secure_port, reds->config->spice_family);
        if (-1 == reds->secure_listen_socket) {
            return -1;
        }
        reds->secure_listen_watch =
            reds_core_watch_add(reds, reds->secure_listen_socket, SPICE_WATCH_EVENT_READ,
...
static int reds_init_ssl(RedsState *reds)
{
    ...
    return_code =
        SSL_CTX_use_certificate_chain_file(reds->ctx, reds->config->ssl_parameters.certs_file);
    if (return_code == 1) {
    ...
    return_code = SSL_CTX_use_PrivateKey_file(
        reds->ctx, reds->config->ssl_parameters.private_key_file, SSL_FILETYPE_PEM);
    if (return_code == 1) {
    ...
    return_code = SSL_CTX_load_verify_locations(
        reds->ctx, reds->config->ssl_parameters.ca_certificate_file, 0);
    if (return_code == 1) {
        spice_debug("Loaded CA certificates from %s",
...
static void migrate_timeout(void *opaque)
{
    ...
    if (reds->mig_wait_connect) {
        ...
        /* in case part of the client haven't yet completed the previous migration,
         * disconnect them */
...
uint32_t reds_get_mm_time(void) { return spice_get_monotonic_time_ms(); }
...
void reds_disable_mm_time(RedsState *reds) { reds->mm_time_enabled = FALSE; }
...
static RedCharDevice *attach_to_red_agent(RedsState *reds, SpiceCharDeviceInstance *sin)
{
    ...
    if (dev->priv->mig_data ||
        ...
        if (!red_char_device_client_exists(RED_CHAR_DEVICE(dev), reds_get_client(reds))) {
            ...
            client_added = red_char_device_client_add(
                RED_CHAR_DEVICE(dev), reds_get_client(reds), TRUE, /* flow control */
                REDS_VDI_PORT_NUM_RECEIVE_BUFFS, REDS_AGENT_WINDOW_SIZE, ~0, TRUE);
...
static void reds_on_char_device_destroy(RedsState *reds, RedCharDevice *dev)
{
...
static int spice_server_char_device_add_interface(SpiceServer *reds, SpiceBaseInstance *sin)
{
    ...
    if (dev_state) {
        ...
        g_object_weak_ref(G_OBJECT(dev_state), (GWeakNotify)reds_on_char_device_destroy, reds);
        /* setting the char_device state to "started" for backward compatibily with
         * qemu releases that don't call spice api for start/stop (not implemented
         * yet) */
        if (reds->vm_running) {
...
SPICE_GNUC_VISIBLE int spice_server_add_interface(SpiceServer *reds, SpiceBaseInstance *sin)
{
    ...
    if (strcmp(interface->type, SPICE_INTERFACE_KEYBOARD) == 0) {
        ...
        if (inputs_channel_set_mouse(reds->inputs_channel,
                                     SPICE_CONTAINEROF(sin, SpiceMouseInstance, base)) != 0) {
            return -1;
        }
        ...
        reds_update_mouse_mode(reds);
        if (reds->is_client_mouse_allowed) {
            inputs_channel_set_tablet_logical_size(reds->inputs_channel, reds->monitor_mode.x_res,
...
#if HAVE_SASL
    int saslerr;
    if ((saslerr = sasl_server_init(NULL, reds->config->sasl_appname ? reds->config->sasl_appname
                                                                     : "spice")) != SASL_OK) {
        spice_error("Failed to initialize SASL auth %s", sasl_errstring(saslerr, NULL, NULL));
        goto err;
    }
#endif
...
static const EnumNames renderer_names[] = {
    {RED_RENDERER_SW, "sw"}, {RED_RENDERER_INVALID, NULL},
};
...
static const int video_codec_caps[] = {
    SPICE_DISPLAY_CAP_CODEC_MJPEG, SPICE_DISPLAY_CAP_CODEC_VP8, SPICE_DISPLAY_CAP_CODEC_H264,
    SPICE_DISPLAY_CAP_CODEC_VP9,
};

/* Expected string:  encoder:codec;encoder:codec */
static const char *parse_video_codecs(const char *codecs, char **encoder, char **codec)
{
    if (!codecs) {
        return NULL;
    }
...
SPICE_GNUC_VISIBLE int spice_server_set_tls(SpiceServer *s,
                                            int          port,
...
{
    if (port == 0 || ca_cert_file == NULL || certs_file == NULL || private_key_file == NULL) {
        return -1;
    }
    if (port < 0 || port > 0xffff) {
        return -1;
...
spice_server_set_channel_security(SpiceServer *s, const char *channel, int security)
{
    int type;
    if (channel == NULL) {
...
SPICE_GNUC_VISIBLE int
spice_server_get_sock_info(SpiceServer *reds, struct sockaddr *sa, socklen_t *salen)
{
    return -1;
}
...
/* very obsolete and old function, retain only for ABI */
SPICE_GNUC_VISIBLE int
spice_server_get_peer_info(SpiceServer *reds, struct sockaddr *sa, socklen_t *salen)
{
    return -1;
}
...
{
    if (value != SPICE_STREAM_VIDEO_OFF && value != SPICE_STREAM_VIDEO_ALL &&
...
uint32_t reds_get_streaming_video(const RedsState *reds) { return reds->config->streaming_video; }
...
GArray *reds_get_video_codecs(const RedsState *reds) { return reds->config->video_codecs; }
...
static int reds_set_migration_dest_info(
    RedsState *reds, const char *dest, int port, int secure_port, const char *cert_subject)
{
...
SPICE_GNUC_VISIBLE int spice_server_migrate_connect(
    SpiceServer *reds, const char *dest, int port, int secure_port, const char *cert_subject)
{
    SpiceMigrateInterface *sif;
    int                    try_seamless;
     ...
     * the client is capable of seamless migration, it is capbable of
     * agent_connected_tokens.
     * The demand for agent_connected_tokens support is in order to assure that if
     * migration
     * occured when the agent was not connected, the tokens state after migration
     * will still
    ...
    /* main channel will take care of clients that are still during migration (at
     * target)*/
    if (main_channel_migrate_connect(reds->main_channel, reds->config->mig_spice, try_seamless)) {
...
SPICE_GNUC_VISIBLE int spice_server_migrate_info(
    SpiceServer *reds, const char *dest, int port, int secure_port, const char *cert_subject)
{
    spice_debug("trace");
    spice_assert(!reds->migration_interface);
...
GArray *reds_get_renderers(RedsState *reds) { return reds->config->renderers; }
...
SpiceCoreInterfaceInternal *reds_get_core_interface(RedsState *reds) { return &reds->core; }

SpiceWatch *
reds_core_watch_add(RedsState *reds, int fd, int event_mask, SpiceWatchFunc func, void *opaque)
{
...
void reds_core_watch_update_mask(RedsState *reds, SpiceWatch *watch, int event_mask)
{
    g_return_if_fail(reds != NULL);
...
SpiceTimer *reds_core_timer_add(RedsState *reds, SpiceTimerFunc func, void *opaque)
{
    g_return_val_if_fail(reds != NULL, NULL);
...
void reds_core_timer_start(RedsState *reds, SpiceTimer *timer, uint32_t ms)
{
    g_return_if_fail(reds != NULL);
...
void reds_core_timer_cancel(RedsState *reds, SpiceTimer *timer)
{
    g_return_if_fail(reds != NULL);
...
void reds_core_timer_remove(RedsState *reds, SpiceTimer *timer)
{
    if (timer == NULL) {
        return;
    }
...
void reds_on_vc_change(RedsState *reds)
{
    ...
    FOREACH_QXL_INSTANCE(reds, qxl) { red_qxl_on_vc_change(qxl, reds_get_video_codecs(reds)); }
}
...
void reds_on_vm_stop(RedsState *reds)
{
    ...
    FOREACH_QXL_INSTANCE(reds, qxl) { red_qxl_stop(qxl); }
}
...
void reds_on_vm_start(RedsState *reds)
{
    ...
    FOREACH_QXL_INSTANCE(reds, qxl) { red_qxl_start(qxl); }
}
...
MainDispatcher *reds_get_main_dispatcher(RedsState *reds) { return reds->main_dispatcher; }
...
static void red_char_device_vdi_port_init(RedCharDeviceVDIPort *self)
{
...
static void red_char_device_vdi_port_finalize(GObject *object)
{
    RedCharDeviceVDIPort *dev = RED_CHAR_DEVICE_VDIPORT(object);
...
static void red_char_device_vdi_port_class_init(RedCharDeviceVDIPortClass *klass)
{
...
static RedCharDeviceVDIPort *red_char_device_vdi_port_new(RedsState *reds)
{
    return g_object_new(RED_TYPE_CHAR_DEVICE_VDIPORT, "spice-server", reds,
                        "client-tokens-interval", (guint64)REDS_TOKENS_TO_SEND, "self-tokens",
                        (guint64)REDS_NUM_INTERNAL_AGENT_MESSAGES, NULL);
}
